...
Golden-set contribution defaults to `on`, in both self-hosted and hosted deployments, clearly disclosed here rather than buried in a settings screen. On this self-hosted instance you can turn it off: set this instance's `GOLDEN_SET_CONTRIBUTION` value to `"off"` — see [/terms](/terms). That variable is real, but no code reads it yet *(design — not yet built; see docs/privacy-claims.md)*, and nothing is contributed by any path today, because the submission client described above does not exist. On the hosted service the same default is not optional: there the labels are the free tier's payment.
