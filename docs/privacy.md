...
- **Anthropic** — receipt images and extracted text, to run the vision extraction call that reads your receipts. This call is built: uploading a photo triggers exactly one of these per receipt. Governed by whichever API key is configured for this instance (your own, in a self-hosted or BYOK deployment).
