 ...
 * (review round 1, finding 5): a sentence stating behaviour that has not
 * been built yet gets an inline `*(design — not yet built; see
 * docs/privacy-claims.md)*` marker at the point of the claim, not just a
 * row in the evidence table nobody reading the page will find. Applied
 * here wherever this document asserts something the claims table marks
 * NOT YET IMPLEMENTED.
 */
export function privacyMarkdown(ctx: LegalContext): string {
  ...
  const anthropicKeyStoredBullet = isHosted
    ? `- **Compute is not billed to you and this instance does not ask you for a personal Anthropic key.** Receipt extraction runs on ${ctx.operatorName}'s own Anthropic API key, with your usage tracked against a monthly per-account extraction budget (see [/terms](/terms)). Your receipt images and extracted text are sent to Anthropic under ${ctx.operatorName}'s account to run that call — not under an account of your own.`
  ...
  const anthropicGovernedBy = isHosted
    ? `${ctx.operatorName}'s own Anthropic API key, tracked per account against a monthly budget (see "What is stored in this instance" above and [/terms](/terms))`
