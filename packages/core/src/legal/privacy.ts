...
export function privacyMarkdown(ctx: LegalContext): string {
  ...
  const anthropicKeyStoredBullet = isHosted
    ? `- **Compute is not billed to you, and this instance does not ask you for a personal Anthropic key.** Receipt extraction runs on ${ctx.operatorName}'s own Anthropic API key: this call is built, and uploading a photo sends your receipt image and extracted text to Anthropic under ${ctx.operatorName}'s account to run it — not under an account of your own. Your usage is intended to be metered against a monthly per-account extraction budget (see [/terms](/terms)) ${UNBUILT_MARKER}: that budget and its metering do not exist in this codebase yet, so today every hosted account's extraction runs with nothing billed to it and no cap on it.`
    : `- **Your Anthropic API key**, if you provide one for receipt extraction, stored as a Cloudflare Workers Secret — a mechanism separate from this instance's database, never written to D1, never logged, and never sent anywhere except to Anthropic's API. This codebase checks that it works (\`GET /api/byok/status\`) and uses it to run the vision-extraction call your photo uploads trigger.`;
